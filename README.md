...
Payload is React and TypeScript with a Vite build, in `src/`, and builds to `site/app/`. Ledger Lane is still a single self-contained HTML file, as is the hub. The older single-file build of Payload is kept at `site/payload.html` as the reference the parity tests diff against.

```bash
npm ci
npm run build                # builds src/ into site/app/
node tools/serve.js          # http://localhost:8000
```

`npm run dev` gives you Vite with hot reload for Payload. Use `tools/serve.js` when testing anything installable: it serves `site/` at the site root, which is the layout the deployed site has, and service workers and the install prompt need http rather than `file://`.

Deploying: Netlify builds from `main` and publishes `site/` as the site root. The settings live in [`netlify.toml`](netlify.toml) — `npm test && npm run build`, publish `site`. The build step is not optional: `site/app/` is gitignored, so a checkout without it serves a hub whose Payload link 404s. Once installed the arcade plays fully offline, both games and fonts included.
...
Play: [`site/payload.html`](site/payload.html)
...
Play: [`site/ledger.html`](site/ledger.html)
